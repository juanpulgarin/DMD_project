...
def plot_band_structure(Title, k_vals, J_list, delta_list,vb,cb, limits,fout=''):
...
def plot_electric_pulse(Title, t, E_t, limits,fout=''):
    fig, axs=plt.subplots(1, figsize=(7,5))
...
def plot_population_field(Title, t_eval,electric_field,k_list,solutions,limits,fout=''):
...
#compare DMD and original result
def plot_dmd_reconstruction_vs_original(t, X_dmd, X_original, k_i=0, Title="DMD vs Original", xlim=None, ylim=None):
    t = t[:X_original.shape[1]]  # Ensure time axis matches data shape
    plt.figure(figsize=(8, 5))

    plt.plot(t, X_dmd[k_i, :], label=f"DMD Reconstruction (k={k_i})", linewidth=2)
    plt.plot(t, X_original[k_i, :], '--', label=f"Original (k={k_i})", linewidth=2)
